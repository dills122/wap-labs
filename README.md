...
| WaveNav runtime (`engine-wasm/`) | Runtime covers deck/head/access/meta parsing, navigation, focus, input/select semantics, deterministic render output, native/WASM parity checks, the completed WML-2 parser/validation baseline, completed WML-302 variable/substitution evidence (20/20 directly mapped clauses), and completed WML-303 action/event/BACK precedence evidence (27/27) | Preserve completed WML-2/WML-302/WML-303 evidence and advance the WML-3 dependency baton through WML-305 without moving fetch behavior into the engine |
...
| WAP evidence program | 30/198 selected parent rows are implemented and 243/762 clauses are directly assessed; Atlas renders the canonical program and active documents | 70 partial and 98 missing parents remain, so the project stays explicitly pre-conformance |
