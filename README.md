...
| WaveNav runtime (`engine-wasm/`) | Runtime covers deck/card parsing, navigation, focus, input/select semantics, deterministic render output, native/WASM parity checks, and the completed WML-201 76-row evidence projection | Map and implement the WML-205 error taxonomy, then close the residual WML-202/WML-204 gates before advancing WML-3 runtime breadth |
...
| WAP evidence program | 22/198 selected parent rows are implemented and 140/761 clauses are directly assessed; Atlas renders the canonical program and active documents | 77 partial and 99 missing parents remain, so the project stays explicitly pre-conformance |
