...
| Lowband transport (`transport-rust/`) | The selected WDP path is 9/9 rows implemented, WCMP is 5/5, TRN-702 constrained-payload/reassembly behavior is direct-fixture-backed, and the pinned WBXML decoder has 46/48 selected WBXML clauses implemented | Close the two explicit WBXML gaps, the selected connectionless WSP evidence, and the TRN-706/707 replay and delta work; activate WTP only with connection-oriented WSP |
| WAP evidence program | 22/201 selected parent rows are implemented and 148/781 clauses are directly assessed; Atlas renders the canonical program and active documents | 78 partial and 101 missing parents remain, so the project stays explicitly pre-conformance |
