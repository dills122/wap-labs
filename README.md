...
|---|---|---|
| Waves desktop app (`browser/`) | Desktop shell is usable end-to-end, and `WBP-00` through additive `WBP-05A` are complete with responsive navigation, host accessibility, and rendered evidence | Keep `WBP-06` implementation-blocked until the remaining `D0-01` contract gate settles; Nokia 7110 planning and Openwave evidence-lock research may proceed without runtime edits |
| WaveNav runtime (`engine-wasm/`) | Runtime covers deck/head/access/meta parsing, navigation, focus, input/select semantics, deterministic render output, native/WASM parity checks, the completed WML-2 parser/validation baseline, and completed WML-303 action/event/BACK precedence evidence (27/27 directly mapped clauses) | Preserve completed WML-2/WML-303 evidence and advance the WML-3 dependency baton through WML-302, then WML-305, without moving fetch behavior into the engine |
...
| WAP evidence program | 24/198 selected parent rows are implemented and 190/762 clauses are directly assessed; Atlas renders the canonical program and active documents | 76 partial and 98 missing parents remain, so the project stays explicitly pre-conformance |
