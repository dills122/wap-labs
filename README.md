...
|---|---|---|
| Waves desktop app (`browser/`) | Desktop shell is usable end-to-end: responsive network/local mode, explicit gateway-probe status, runtime deck navigation, focused text/select editing, browser back/reload flow, debug/timeline surfaces, an ordinary-browser story adapter backed by the real WASM engine, and the first `WBP-01` component seams | Close the remaining `WBP-00`/`WBP-01` baseline and integration evidence, then fan out browser-owned visual, toolbar, onboarding, and accessibility slices without changing engine or transport contracts |
| WaveNav runtime (`engine-wasm/`) | Runtime covers deck/head/access/meta parsing, navigation, focus, input/select semantics, deterministic render output, native/WASM parity checks, the completed WML-201 76-row evidence projection, WML-205 structured load diagnostics, 30/30 completed WML-202 clauses, and 23/23 mapped WML-204 clauses | Preserve WML-202 closure; finish WML-204 conversion/validation, WML-203 prologue/DTD, and WML-205 enforcement/atomicity gates serially before advancing WML-3 runtime breadth |
...
| WAP evidence program | 23/198 selected parent rows are implemented and 161/761 clauses are directly assessed; Atlas renders the canonical program and active documents | 77 partial and 98 missing parents remain, so the project stays explicitly pre-conformance |
