...
| WaveNav runtime (`engine-wasm/`) | Runtime covers deck/head/access/meta parsing, navigation, focus, input/select semantics, deterministic render output, native/WASM parity checks, and the completed WML-2 parser/validation baseline: WML-201 (175/175 mapped clauses), WML-202 (30/30), WML-203 (68/68), WML-204 (23/23), and WML-205 (3/3) | Preserve WML-2 closure and advance the WML-3 dependency baton through WML-303, WML-302, and WML-305 without moving fetch behavior into the engine |
...
| WAP evidence program | 24/198 selected parent rows are implemented and 176/762 clauses are directly assessed; Atlas renders the canonical program and active documents | 76 partial and 98 missing parents remain, so the project stays explicitly pre-conformance |
