...
provides the local foundation for the desktop-only public WAP lab. A hardened release is deployed
to the restricted preview host with Tailnet-only access and sealed public ingress; public DNS and
UDP 9200 publication remain separately gated. The service is not embedded in the desktop
application or connected to the browser-hosted WASM simulator.
...
(updated 2026-07-30; audited at `6cf1682a`).

| Track                                            | Implemented                                                                                                                                                                                                                                                                                                                                                                                                            | Roadmap / In Progress                                                                                                                                             |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Waves desktop app (`browser/`)                   | Desktop shell is usable end-to-end; the canonical Canvas frame path, deterministic frame/navigation sequencing, cancellable navigation, native command registry, versioned application state, safe Favorites domain, bounded typed host ingress, truthful navigation errors, safe diagnostic projections, and the bounded D0-03 debug host bridge are landed alongside the completed shell/accessibility foundation    | Complete bounded host UI state, deterministic click/scroll/softkey input, the read-only D0-04 Inspector, phase-aware recovery, and integrated Library/Preferences |
| WaveNav runtime (`engine-wasm/`)                 | Runtime covers deck/head/access/meta parsing, navigation, focus, input/select semantics, deterministic bounded render output, native/WASM parity checks, the completed WML-2 parser/validation baseline, completed WML-302 variable/substitution, WML-303 action/event/BACK, WML-305 timer, WML-309 frame/affordance contract, WMLS-501 strict verifier evidence, and the bounded D0-02 debug recorder/snapshot source | Close replayable POST history and multipart residuals, advance WMLS-502 execution, and connect the D0-04 Inspector consumer                                       |
| Lowband transport (`transport-rust/`)            | The selected WDP/WCMP path and WBXML decoder have direct evidence; WSP-801/WSP-802 close the selected connectionless PDU/header foundation, WSP-805 applies the typed WML request intent through native serialization, and the shared Lowband facade now powers the `wapcurl` diagnostic CLI                                                                                                                           | Close replayable POST history and remaining WML-304/WSP-805 evidence; activate WTP only with connection-oriented WSP                                              |
| WAP evidence program                             | 41/198 selected parent rows are implemented and 317/762 clauses are directly assessed; Atlas renders the canonical program and active documents                                                                                                                                                                                                                                                                        | 78 partial and 79 missing parents remain, so the project stays explicitly pre-conformance                                                                         |
| Frame-based render/input migration               | `F0-01` through `F0-03` and `F1-01` through `F1-03` provide canonical frame/input types, host-sample and Canvas renderers, deterministic navigation publication, generated projections, and drift checks                                                                                                                                                                                                               | Continue F2 typed pointer/scroll/softkey input, F3 layout/paint parity, semantic accessibility, and the final F4 legacy-path removal                              |
...
| WAP lab stack (`gateway-kannel/`, `wml-server/`) | The bounded Go origin and hardened Kannel deployment are healthy on the restricted preview host; Tailnet WAP GET/POST, unknown-origin denial, sealed-firewall persistence, reboot recovery, and rollback retention are evidenced                                                                                                                                                                                       | Keep ingress sealed until access/threat decisions, exact DNS/firewall plan approval, public probes, kill-switch rehearsal, and release gates close                |
