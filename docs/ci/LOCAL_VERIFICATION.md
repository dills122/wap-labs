...
| Full     | `pnpm verify:full`                    | Runs every deterministic offline lane, including compliance/status drift, graph drift, native and WASM engine checks, contracts, stories, transport, browser host/frontend/rendered accessibility, Atlas, marketing, and WML server checks. |
...
`./scripts/init-refresh.sh` to install or refresh the repository-managed Node, pnpm, wasm-pack,
and workspace dependencies before retrying.
