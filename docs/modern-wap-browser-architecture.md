...
1. Tauri host talks only to transport HTTP API (no UDP in UI process).
2. Tauri host talks to the WaveNav WASM engine through in-process function contracts.
...
- Keep contracts backward-compatible once consumed by the desktop host.
