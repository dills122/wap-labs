...
| `RQ-SEC-004`, `RQ-SEC-005` WTLS posture and security delegation behavior | `partial` | Baseline evidence is in `transport-rust/src/network/wtls/record.rs`, `transport-rust/src/network/wtls/alerts.rs`, and `transport-rust/src/network/wtls/handshake.rs` with fixture corpora `transport-rust/tests/fixtures/transport/wtls_record_boundary_mapped/record_fixture.json` and `transport-rust/tests/fixtures/transport/wtls_handshake_reliability_mapped/handshake_fixture.json`; command: `cd transport-rust && cargo test --lib`; `T0-21` closes the explicit disabled-vs-active-minimal boundary, while full certificate and crypto parity remains deferred |
