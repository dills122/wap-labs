...
| `RQ-TRN-014` WSP encoding-version and header encoding rules | `partial` | Deterministic encoding-version negotiation, text fallback, and unsupported binary status output are covered in `transport-rust/src/network/wsp/encoding_version.rs` plus header code-page handling in `transport-rust/src/network/wsp/decoder.rs` and `transport-rust/src/network/wsp/encoder.rs`; command: `cd transport-rust && cargo test --lib` |
