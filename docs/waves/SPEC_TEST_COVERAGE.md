...
| `RQ-TRN-018` WSP assigned-number registry fidelity | `partial` | Deterministic decode/encode + unknown-policy tests landed in `transport-rust/src/wsp_registry.rs` and `transport-rust/tests/fixtures/transport/wsp_assigned_number_registry_mapped/`; additional protocol-path integration remains planned under `T0-20`; command: `cd transport-rust && cargo test --lib` |
