...
| Effective WAP-192 + SIN 105 WBXML SCR closure (15 actor-specific IDs) | `partial` | SCR ledger: `spec-processing/source-manifests/wap-1.2.1-wbxml-scr.json`; clause ledger: `spec-processing/source-manifests/wap-1.2.1-selected-normative-clauses.json`; `WBXML:MCF` selects 3 rows expanded into 48 anchored clauses; 35 fixed-outcome fixtures cite 44 clauses, 42 are implemented, 6 remain not assessed, and the parent-row audit stays 0 implemented / 3 partial / 0 missing |
...
| Story-driven browser replay | `partial` | `pnpm test:story all` executes typed companion flows for basic fragment/external intent, history back/empty history, zero-timer dispatch, and W0-05 timer→script→dialog publication against the production WASM host; transport and native Tauri replay remain open |
...
| `RQ-RMK-010` WBXML 1.3 client decoder conformance | `partial` | Pinned decoder: `transport-rust/src/wbxml_decoder.rs`; 35-case direct corpus: `transport-rust/tests/fixtures/transport/wbxml_wml13/conformance.json`; exact parent tests plus `transport_wbxml_section_5_direct_clause_inventory_is_fixed_outcome_backed`; engine parity: `wml_203_canonical_wml13_doctype_and_decoded_wbxml_reach_equal_decks`; six explicit clause gaps remain in `R0-08`/`WML-203` |
| `RQ-TRX-006..008` WCMP handling | `covered` | Selected general-WCMP codec, generation/handling, bounded payload policy, error mapping, and direct fixture evidence are in `transport-rust/src/network/wcmp/` and `transport-rust/tests/fixtures/transport/wcmp_core_mapped/`; command: `cargo test --manifest-path transport-rust/Cargo.toml --lib network::wcmp`; unselected optional/server WCMP breadth remains capability-gated |
