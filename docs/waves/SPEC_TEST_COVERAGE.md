...
| Effective WAP-192 + SIN 105 WBXML SCR closure (15 actor-specific IDs) | `partial` | SCR ledger: `spec-processing/source-manifests/wap-1.2.1-wbxml-scr.json`; clause ledger: `spec-processing/source-manifests/wap-1.2.1-selected-normative-clauses.json`; `WBXML:MCF` selects 3 rows expanded into 48 anchored clauses; 36 fixed-outcome fixtures cite 47 clauses, 46 are implemented, 2 remain not assessed, and the parent-row audit stays 0 implemented / 3 partial / 0 missing |
...
| `RQ-RMK-010` WBXML 1.3 client decoder conformance | `partial` | Pinned decoder: `transport-rust/src/wbxml_decoder.rs`; 36-case direct corpus and exhaustive WML page-zero equivalence matrix: `transport-rust/tests/fixtures/transport/wbxml_wml13/conformance.json`; exact parent and matrix tests; engine parity: `wml_203_canonical_wml13_doctype_and_decoded_wbxml_reach_equal_decks`; two explicit clause gaps remain in `R0-08`/`WML-203` |
