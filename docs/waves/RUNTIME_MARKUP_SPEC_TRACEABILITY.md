...
- AC:
  - Evidence: [x] WML-303 task identity, optional/noop filtering, card/template shadowing, intrinsic conflicts/scope, BACK/accept precedence, entry order, rollback, and the distinction between a handled unchanged-card override and an empty-history `<prev/>` override are covered in `engine-wasm/engine/src/engine_tests/wml_303_actions.rs`, the native/WASM host boundary suites, the native Waves hybrid-Back scenarios, and `engine-wasm/examples/source/wml-303-actions-softkeys.flow.json`. The host-only `navigateBackToCard` boundary restores an older same-deck host entry with backward entry semantics and no new engine-history push or transport refetch; run `cargo test --manifest-path engine-wasm/engine/Cargo.toml wml_303`, `wasm-pack test --node engine-wasm/engine`, and `pnpm test:story WML-303`.
