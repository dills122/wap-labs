...
| `postfield`/`setvar` behavior | 9.3-9.4 | parser + runtime + transport | partial | WML-304 resolves ordered postfields, serializes fields/charsets at the native transport boundary, and replays typed POST values from history | Close multipart per-part Content-Type construction |
