...
| `postfield`/`setvar` behavior | 9.3-9.4 | parser + runtime + transport | partial | WML-304 parses and resolves ordered postfields into a typed request intent and suppresses same-deck postfields unless no-cache is requested | Consume the intent at the native transport boundary, serialize fields/charsets, and preserve replayable POST data |
