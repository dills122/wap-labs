...
- AC:
  ...
  - Evidence: [x] WML-302 explicitly closes `HISTORY-RESOLVES-VARIABLES` at the engine boundary: WML-authored targets are resolved before same-deck history insertion or external navigation intent publication. WML-301 retains broader history-model ownership, while WML-304 retains GET/POST/postfield request construction and request-shaped history persistence.
