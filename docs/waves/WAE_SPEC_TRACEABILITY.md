...
- AC:
  ...
  - Evidence: [x] WML-301 closes ordered request-shaped history, duplicate explicit accesses, entry-field retention without card content, cross-deck context continuity/replacement, and destination push order in `browser/frontend/src/session-history.test.ts`, `browser/frontend/src/app/navigation-state.history.test.ts`, `browser/frontend/src/app/navigation-state.load.test.ts`, and `engine-wasm/examples/source/wml-301-context-history.flow.json`; run `pnpm --dir browser/frontend test` and `pnpm test:story WML-301`.
  - Evidence: [x] WML-301's focused graph labels `WAE-CL-WML-LANGUAGE-DELEGATE`, `WAE-CL-WML-USER-AGENT-COMPOSITION`, and `WAE-CL-WMLSCRIPT-LANGUAGE-DELEGATE` as aggregate context only; all three remain `not-assessed` and receive no completion claim from the card/table fixture.
  - Evidence: [x] WML-302 explicitly closes `HISTORY-RESOLVES-VARIABLES` at the engine boundary before same-deck history insertion or external navigation handoff. WML-304 retains GET/POST request construction and postfield serialization; Request A2 replays the retained typed POST values when Back refetches a prior deck.
