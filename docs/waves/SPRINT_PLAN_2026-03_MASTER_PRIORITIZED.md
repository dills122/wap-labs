...
|        5 | Request A2 — replayable POST history (complete)                                    | Retains typed semantic POST input and replays it through transport on explicit Back without preserving derived legacy `postContext` bytes as the authoritative identity.                                         | Closes `WML-CL-HISTORY-POST-REPLAY`, bringing WML-304 to 14/15 and WML-C-07/WML-C-38 to implemented.                         | Makes form navigation/back behavior predictable in public-lab scenarios.                       |
...
`WML-306` is the next compliance slice after this batch now that A1/A2 no longer own its browser
policy files. The public-WAP-services workstream
