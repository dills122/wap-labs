...
The six-task batch is retained as dispatch history. A1, A2, B1, C1, and D1 are complete; B2 is
the active compliance baton. The desktop pre-release lane has additionally completed F1 host
...
| -------: | ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
|        1 | Request A1 — `WML-304` / `R0-06` / `WSP-805` native request application (complete) | Landed typed GET query/form-urlencoded POST construction, charset-bearing content type, referer/no-cache policy, and direct HTTP/native WSP handoff while preserving the transport/browser ownership boundary. The additive closure now supplies deterministic multipart parts. | Brought WML-304 to 13/15 before A2; typed replay and multipart per-part typing subsequently close it at 15/15.             | Makes real network forms and request policy usable through the native desktop path.            |
...
|        5 | Request A2 — replayable POST history (complete)                                    | Retains typed semantic POST input and replays it through transport on explicit Back without preserving derived legacy `postContext` bytes as the authoritative identity. The additive multipart closure completes the remaining transport clause.                            | Closes `WML-CL-HISTORY-POST-REPLAY`; multipart per-part typing then brings WML-304 to 15/15 without overstating broader WML-C-29 closure. | Makes form navigation/back behavior predictable in public-lab scenarios.                       |
...
WML-304 and WML-306 are complete after this batch. The next fresh WML-3 residual slices are
WML-307 and WML-308; WML-301's aggregate WAE delegates remain separate. The public-WAP-services workstream
