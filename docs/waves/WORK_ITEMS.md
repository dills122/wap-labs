...
- Remaining scope: `do`/`onevent`, the optional `optgroup` capability declaration, `timer`, general vdata/HREF validation and conversion outside the bounded input/select paths, and language-aware non-Basic-Latin mask repertoires. The mandatory Unicode Basic Latin input floor is implemented and directly tested.
