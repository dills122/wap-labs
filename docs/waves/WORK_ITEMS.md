...
- execution index:
  ...
  - `NE2E-02` through `NE2E-04`: modular runner, fresh scenario sessions, Waves interaction API,
    WebDriver status readiness, cancel-aware bounded waits, and configured retry-horizon stability —
    implemented
  - `NE2E-05`: fail-closed exact safe-upload evidence, static post-initialization/pre-scenario
    infrastructure failure bundles, validator-gated uploads, fixed boundary checkpoints/failure
